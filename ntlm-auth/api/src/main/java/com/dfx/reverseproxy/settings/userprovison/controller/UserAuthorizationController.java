...
public class UserAuthorizationController {
	...
	public ResponseEntity<?> createUserGroup(@RequestBody DFXUserGroups userGroups) {
		...
		try {
			...
			HttpStatus returnStatus = HttpStatus.CREATED;
			TBL_DMS_USERS users = usersRepository.findByLogonid(userGroups.getLogonId());

			if (users != null) {
				List<TBL_DMS_User_Groups> existingUserGroups = userGroupsRepository.findByLogonId(logonId);
				List<TBL_DFX_User_Groups> existingDfxUserGroups = dfxUserGroupsRepository
						.findByLogonIdAndDeletedRecord(logonId, false);
				...
				existingdfxGroupNames.stream().filter(groupName -> !newGroupNames.contains(groupName))
						.forEach(groupName -> {
							deleteUsergroup(logonId, groupName);
						});

				newGroupNames.stream().filter(groupName -> !existingGroupNames.contains(groupName))
						.forEach(groupName -> {
							saveUsergroup(groupName, logonId, users.getDomainUser());
						});
	...
	private void deleteUsergroup(String logonId, String groupName) {
		TBL_DFX_User_Groups dfxUserGroups = dfxUserGroupsRepository.findByLogonIdAndGroupNameAndDeletedRecord(logonId,
				groupName, false);
	...
	private void saveUsergroup(String groupName, String logonId, String domainUser) {
		Date dNow = new Date();
		...
		userGroup.setDomainUser(domainUser);
	...
	public ResponseEntity<?> retrieveUsers(@RequestBody() Map<String, String> Payload) {
		try {
			String userName = Payload.get("filtervalue");
			List<TBL_DMS_USERS> users = usersRepository.findByLogonidContainingIgnoreCase(userName);
			JSONArray userLists = new JSONArray();
			for (TBL_DMS_USERS user : users) {
				userLists.put(user.getLogonid());
			}
	...
	@RequestMapping(method = {
			RequestMethod.GET }, value = "/group/{groupName}", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<?> retrieveUserForGroup(@PathVariable("groupName") String groupName) {
		try {
			...
			if (dfxGroup != null || groups != null) {
				...
				List<TBL_DFX_User_Groups> dfxUserGroups = dfxUserGroupsRepository
						.findByGroupNameAndDeletedRecord(groupName, false);
	...
	public ResponseEntity<?> retrieveGroupsForUser(@RequestBody() Map<String, String> Payload) {
		try {
			...
			List<TBL_DFX_User_Groups> dfxUserGroups = dfxUserGroupsRepository.findByLogonIdAndDeletedRecord(logonId,
					false);
