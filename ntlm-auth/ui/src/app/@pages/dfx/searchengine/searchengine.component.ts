...
export class SearchengineComponent implements OnInit, OnDestroy {
  ...
  resolveTypes = [{key:1, value:'Resolved Client'},
  {key:3, value:'Auto Resolved'},
  {key:0, value:'Unresolved'},
  {key:-1, value:'Ignored'}]
  ...
  searchAggregationResolve: any;
  aggregateResolveType: any = [];
  ...
  setBody(keyword, getDate) {
    this.body = {
      ...
      aggs: {
        ...
        by_resolvetype: { terms: { field: "resolve_type" } },
        groups: {
          date_histogram: {
            field: "createddate",
            interval: "month",
    ...
    const resolveterm = { terms: { "resolve_type": this.resolveArray || [] } };
    ...
    this.removeEmptyKeyFromObject(resolveterm);
    const Obj = [
      authterm,
      cpcoterm,
      locaterm,
  ...
  getAggregation(response) {
    ...
    this.aggregateType = response.aggregations.by_pdtype.buckets
      ...
      .map((data) => data.key);
    // Get Resolve
    this.aggregateResolveType = response.aggregations.by_resolvetype.buckets
    // Get Migrated
  ...
  valueSelected(event: any, value: any, filterName: any) {
    console.log(event,value,filterName);
    this.pageCount = 1;
    ...
    switch (filterName) {
      ...
      case "Resolve":
        {
        if (event.target.checked && !this.resolveArray.includes(value)) {
          this.resolveArray = [...this.resolveArray, value];
        } else if (!event.target.checked && this.resolveArray.includes(value)) {
          {
            this.resolveArray = this.resolveArray.filter((data) => data !== value);
          }
        }
      }
          break;
  ...
  removeFromFilter(tag: any) {
    ...
    this.resolveArray = this.resolveArray.filter((dt) => dt !== tag);
    if (tag === "migrated") {
      this.migrationSuccess = false;
    }
    if (tag === "nonmigrated") {
  ...
  resolvetyped(key){
     return this.resolveTypes.find(rsType=>rsType.key == +key).value
  }
}
